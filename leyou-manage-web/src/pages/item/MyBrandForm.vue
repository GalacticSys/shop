<template>
      <v-form v-model="valid" ref="BrandForm">
        <v-text-field v-model="brand.name" label="品牌名称" required :rules="myRules.nameRules">
        </v-text-field>
        <v-text-field v-model="brand.letter" label="品牌首字母" required mask="A" :rules="myRules.letterRules"></v-text-field>
        <v-cascader
          ...
          return {
            ...
            brand:{
              ...
              myRules:{
                ...
                letterRules:[
                    v=>!!v || "首字母不能为空",
                    v=> /^[a-zA-Z]{1}$/.test(v) || "品牌首字母只能是一个字母"
                ]
              },
          }
        },
