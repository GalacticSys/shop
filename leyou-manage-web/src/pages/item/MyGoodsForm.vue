    ...
    <v-stepper-items>
      ...
      <v-stepper-content step="3">
        <v-flex class="xs10 mx-auto px-3">
            <v-card v-for="spec in specifications" :key="spec.group.id" class="my-2">
              <v-card-title class="subheading"><h4>{{spec.group.name}}</h4></v-card-title>
                <v-divider></v-divider>
                <v-card-text v-for="param in spec.params" :key="param.id" v-if="param.generic" class="px-5">
                  <!--判断是否有可选项，如果没有，则显示文本框。还要判断是否是数值类型，如果是则把unit显示到后缀-->
                  <v-text-field v-if="param.segments.length <= 0" :label="param.name" v-model="param.v" :suffix="param.unit || ''"></v-text-field>
                  <!--否则，显示下拉选项-->
                  <v-select v-else :label="param.name" v-model="param.v" :items="param.segments"/>
                </v-card-text>
            </v-card>
        </v-flex>
      </v-stepper-content>
      ...
      <v-stepper-content step="4">
        <v-card v-for="spec in specialSpecs" :key="spec.id">
<!--          <v-card-title v-if="spec.length <= 0" class="subheading">-->
<!--            <h4 class="xs3">{{spec.name}}</h4>-->
<!--            <v-spacer></v-spacer>-->
<!--            &lt;!&ndash;-->
<!--                如何控制文本框的增加与删除-->
    ...
    export default {
      ...
      methods:{
        ...
        dataProces(temp){
          ...
          //获取sku特有属性
          temp.forEach(({params}) => {
            params.forEach(({id, name,numeric,generic,unit,segments}) => {
              if (!generic) {
                specialTemplate.push({
                  id, name, numeric,generic,unit,segments
                })
              }
            })
          });
          ...
          for (let i=0;i<temp.length;i++){
            if (temp[i].params.length > 0){
              ...
              for (let j = 0;j < temp2.length; j++){
                if (temp2[j].segments.length>0){
                  let arr = temp2[j].segments.split(",");
                  temp2[j].segments=arr;
                }
                if (temp2[j].generic){
                  param.push(temp2[j]);
                }
              ...
              if (count !== temp2.length && param.length !== 0) {
                commonTemplate.push({
                  group: {id:temp[i].id,cid:temp[i].cid,name:temp[i].name},
                  params: param,
                });
              }
            }
