        ...
        <v-card>
          ...
          <v-card-text>
            <v-container fluid grid-list-md>
              <v-layout row wrap>
                <v-flex xs12>
                  <v-card v-for="(spec,index) in specGroups" :key="index" class="my-3">
                    <v-card-title>
                      <strong><input type="text" style="font-size: 16px" v-model="spec.name"
                      ...
                      <v-expansion-panel inset>
                        <v-expansion-panel-content v-for="(param,i) in spec.params" :key="i">
                          <v-layout wrap row slot="header">
                            <input placeholder="请输入属性名" size="15" v-model="param.name" @click.stop=""/>
                            <v-spacer/>
                          ...
                          <v-card class="mx-5">
                            <v-layout row>
                              <v-checkbox color="success" v-model="param.searching" label="是否是可搜索"/>
<!--                              <v-checkbox color="success" v-model="param.global" label="是否全局属性"/>-->
                            </v-layout>
                            <v-layout row>
                              <v-checkbox color="success" v-model="param.numeric"  label="是否数值类型"/>
                              <v-text-field v-show="param.numeric" label="计量单位" v-model="param.unit" />
                            </v-layout>
                            <v-card-text v-if="param.length > 0">
                              <v-layout row v-for="j in param.length" :key="j">
                                <input placeholder="请输入选项值" v-model="param[j-1]"/>
                                <v-spacer/>
...
//  import {treeData,phoneSpec} from "../../mockDB";
  ...
  export default {
    ...
    methods: {
        ...
        if (!node.isParent) {
          ...
          this.$http.get("/item/spec/" + node.id)
            .then(resp => {
              ...
              // 查询成功后，把响应结果赋值给specGroups属性，Vue会进行自动渲染。
      ...
      // 为属性添加默认值
      addOption(i, j) {
        this.specGroups[i].params[j].options.push("")
      },
      // 保存、修改、删除模板
      saveTemplate() {
        this.dialog = true;
          ...
          // this.verify().then(() => {
            this.$http({
              ...
              data: this.$qs.stringify({
                categoryId: this.currentNode.id,
                specGroups:JSON.stringify(this.specGroups)
              })
            }).then(() => {
              this.dialog = false;
      ...
      deleteGroup(i) {
        this.specGroups.splice(i, 1)
      },
      // 删除指定模板
      deleteParam(i, j) {
        this.specGroups[i].params.splice(j, 1);
      },
      // 删除指定值
      deleteOption(i1, i2, i3) {
        this.specGroups[i1].params[i2].options.splice(i3, 1);
      },
