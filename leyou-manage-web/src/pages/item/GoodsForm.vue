  ...
  <v-stepper v-model="step">
    ...
    <v-stepper-items>
      ...
      <v-stepper-content step="1">
        <v-flex class="xs10 mx-auto">
          <v-form v-model="valid" ref="basic">
            ...
            <v-text-field label="商品名称" v-model="goods.title" :counter="200" required :rules="[v => !!v || '商品标题不能为空']" hide-details/>
      ...
      <v-stepper-content step="2">
        <v-editor v-model="goods.spuDetail.description" uploadUrl="/upload/image" needSignature/>
      </v-stepper-content>
      <!--3、规格参数-->
            ...
            <v-data-table :items="skus" :headers="headers" hide-actions item-key="indexes" class="elevation-0">
              ...
              <template slot="expand" slot-scope="props">
                <v-card class="elevation-2 flex xs11 mx-auto my-2">
                  <!--图片上传组件-->
                  <v-upload v-model="props.item.images" url="/upload/image" needSignature/>
                </v-card>
              </template>
            </v-data-table>
          </v-card>
        ...
        if (val && val.length > 0) {
          ...
          this.$http
            ...
            .then(({ data }) => {
              ...
              //清空品牌选项
              this.clearBrandOptions()

          );
          // 根据分类查询规格参数
          this.$http
            .get("/item/spec/param/" + this.goods.categories[2].id)
            .then(({ data }) => {
              let specs = [];
              let template = [];
              if (this.isEdit){
              ...
              // console.log("this.specs通用规格");
              // console.log(arr1);
              // //
              // console.log("this.specialSpecs特有规格");
              // console.log(arr2);
