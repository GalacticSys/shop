    ...
    <v-dialog max-width="900" v-model="show" persistent scrollable>
      <v-card>
        ...
        <v-card-text class="px-5" style="height: 600px">
          <goods-form @initStep="initStep" @close="close" :isEdit="isEdit" :oldGoods="oldGoods" :step="step" :show="show"  ref="goodsForm"/>
        </v-card-text>
        <v-card-actions>
      ...
      methods:{
        ...
        editGoods(oldGoods){
          ...
          this.$http.get("/item/goods/spu/"+oldGoods.id).then(({data}) => {
            this.isEdit = true;
            this.oldGoods.skus=data.skus;
            this.oldGoods.skusList = data.skus;
            this.oldGoods.spuDetail = data.spuDetail;
            this.oldGoods.spuDetail.specTemplate = JSON.parse(data.spuDetail.specialSpec);
            this.oldGoods.spuDetail.specifications = JSON.parse(data.spuDetail.genericSpec);
